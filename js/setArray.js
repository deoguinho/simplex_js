...
function checkInputGraphic() {
	if (num_vars != 2)
		alert('as variáveis ​​de decisão devem ser iguais a 2');
	else if (checkInput())
		graphic();
}
function clearInput() {
...
function setArray() {
	...
	setAttributes(tographic, {'type':'button', 'value':'Gráfico →', 'class':'nextstep', 'onclick': 'checkInputGraphic()', 'style': "margin-top: 15px;"});
	container.appendChild(tographic);
	let clearInput = document.createElement('input');
	setAttributes(clearInput, {'type':'button', 'value':'Limpar', 'class':'nextstep', 'onclick': 'clearInput()', 'style': "margin-top: 15px;"});
	container.appendChild(clearInput);
	let back = document.createElement('input');
	setAttributes(back, {'type':'button', 'value':'← voltar', 'class':'nextstep', 'onclick': 'backPage()', 'style': "margin-top: 15px; float: left"});
