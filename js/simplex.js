...
function createSaver() {
	...
	setAttributes(input, {'type':'button','value': 'Imprimir','onclick': 'window.print()', 'style':'color: white;background-color: #022022;'});
...
function simplex()
{
	...
	setAttributes(back, {'type':'button', 'value':'← voltar', 'class':'nextstep', 'onclick': 'backPage2()', 'style': "float: left; margin-top: 15px;"});
	...
	printArray(tab, width, height, -1, -1, is_phase != 0 ? "Tabela inicial da fase I" : "Tabela inicial");
	if(is_phase != 0) {
		...
		while(check_negative(tab, height, width)) {
			...
			printArray(tab, width, height, point[0], point[1], "próximo");

			tab = gauss(tab,height , width, point[0], point[1], choix, 1);
		}
		printArray(tab, width, height, -1, -1, "Tabela final da fase 1");
		...
		printArray(tab, width, height, -1, -1, "Tabela inicial da fase 2");
		correction(tab, width, height);
		printArray(tab, width, height, -1, -1, "Correção");
	}
	...
	while(check_pos_neg(tab, height, width, choix)) {
		...
		if (already === 1) {
			printArray(tab, width, height, point[0], point[1], "Esta é a Tabela inicial => Ciclico");
			already = 2;
		}
		else if (already === 2) {
			printArray(tab, width, height, point[0], point[1], "Regra de Bland");
			already = 0;
		}
		else
			printArray(tab, width, height, point[0], point[1], "próximo");
		...
		if (++init > 1 && is_equal(tab, tab_init, width - 1, height)) {
			console.log('é cíclio');
			bland = true;
			already = 1;
		}
	}
	printArray(tab, width, height, -1, -1, "Tabela final");
	createSaver();
}
